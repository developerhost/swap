...
2. Recommendedの拡張機能インストール(vscode)
3. node.js LTSをインストールする <https://nodejs.org/en/download/>
4. 使用するnode.jsバージョンを18.17.0以上に設定する
5.   参考　https://qiita.com/Sr_Bangs/items/bda855068576905a441f
6. pnpmをインストールする `npm install -g pnpm`
7. パッケージをインストールする `pnpm install`
8. [環境変数最新チャンネル](https://discord.com/channels/1173435345354887198/1173435866522329108)を参照し、最新の環境変数の値をもとに.envファイルをcloneしたディレクトリの直下に作成する
9. `pnpm prisma generate`を実行する
10. `pnpm run dev`を実行し、localhost:3000にアクセスして動作すればOK
11. PlanetScale,vercel, google analytics, S3の中身を見る必要がある方はメールアドレスをdiscordで橋田に送ってください。プロジェクトに招待します。
...
8. [PlanetScale](https://planetscale.com/)
9. [vercel](https://vercel.com/dirtyman69/media-swap)
10. ドメイン[swappy](https://www.swappy.jp/)

~~## 各自がリモートのmongoDBを作成する手順(ローカル開発環境ではなく、こちらを実行してください)~~
→上記は今後mongoDBではなく、PlanetScaleを使います。ローカルの開発環境にDockerを使用してmySQLを立ち上げてください

## DockerでmySQLを立ち上げる方法

### 1. mySQLクライアントがない場合はinstall
...
### 2. media-swapのルートディレクトリにて以下コマンド実行
```
docker-compose up -d
```
...
# パスワードを聞かれるので、swappyと入力(ユーザー:user,パスワード:passwordを使っても良い→ルートユーザーで入るか、作成したユーザーで入るかの違い)
...
にてprisma generateとdb pushを行い、スキーマが正しく定義されているか確認する
...
| transfer_requests         |
| user_coupons              |
| user_notification_permits |
| user_notification_reads   |
| user_transaction_ratings  |
| users                     |
| verification_tokens       |
+---------------------------+
...
### 補足
sqlのURLはenvのDATABASE_URLに設定してある。userやpasswordの名前を変えたかったら各自修正する

## 参考記事

https://zenn.dev/ryo7/articles/create-mysql-on-docker

# 以下mongoDBのセットアップは今後行う必要なし！
