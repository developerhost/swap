...
export const useItemCommentReportModal = (commentId: string | null) => {
  ...
  const reportItemComment = useCallback(
    async (f: FormData) => {
      ...
      if (!sessionUser) {
        toast.error(MESSAGE.ERROR.SESSION_TIMEOUT);
        return;
      }
      ...
      if (!commentId) {
        toast.error(MESSAGE.ERROR.COMMENT_NOT_SELECTED);
        return;
      }

      if (reason.length < 3) {
        toast.error(MESSAGE.ERROR.TEXT_MIN_LENGTH(3));
        return;
      }

      if (reason.length > 1000) {
        toast.error(MESSAGE.ERROR.TEXT_MAX_LENGTH(1000));
        return;
      }
      ...
      if (!verificationCode) {
        toast.error(MESSAGE.ERROR.VERIFICATION_FAILED);
        return;
      }

      const reportResult = await addItemCommentReport(
        commentId,
        reason,
        verificationCode,
      );
      ...
      toast.success(MESSAGE.SUCCESS.REPORT_COMMENT);
    },
