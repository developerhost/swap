...
import { type Merge, type URLString } from "@/utils/types";
...
/** 出品情報編集フォームの状態とバリデーション、メッセージを表す型 */
export type EditProductFormState = FormState<
  Merge<ProductFormValues, { id: string }>
>;
...
export const ProductFormSchema: ZodType<ProductFormValues> = z
  .object({
    ...
    tags: z.string().refine(isNgWord, {
      message: "商品関連キーワードにNGワードが含まれています",
    }),
    verificationCode: z
